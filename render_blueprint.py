#!/usr/bin/env python3
"""Render banner 7C (Julie · Longevity Blueprint) at every Meta / Instagram accepted size."""
import os, json
...
with sync_playwright() as p:
    ...
    for f, (w, h, tag) in SIZES.items():
        for guide in (0, 1):
            ...
            page.goto(f'file://{ROOT}/blueprint.html?f={f}&guide={guide}')
            page.wait_for_function('window.__ready===true', timeout=30000)
            page.wait_for_timeout(350)
            m = page.evaluate('window.__meta')
            sub = 'guides' if guide else 'ads'
            name = f'LLA_BLUEPRINT_{tag}' + ('_SAFEZONE' if guide else '') + '.png'
            page.screenshot(path=os.path.join(ROOT, sub, name),
                            clip={'x': 0, 'y': 0, 'width': w, 'height': h})
            page.close()
            if not guide:
                report.append({'banner': 'blueprint', 'format_key': f, 'file': f'{sub}/{name}',
                               'output_w': w, 'output_h': h, 'aspect': round(w / h, 4),
                               'safe_reserve_top_px': m['safeTop'],
                               'safe_reserve_bottom_px': m['safeBottom'],
                               'safe_reserve_side_px': m['insetX'],
                               'all_copy_inside_safe_zone': m['fits'],
                               'card_render_scale': round(m['sc'] * m['S'], 4),
                               'source_photo': m['photoSrc'],
                               'source_photo_px': f"{m['photoW']}x{m['photoH']}",
                               'photo_cover_scale': m['photoUpscale']})
            print('rendered', name, 'safe=' + str(m['fits']), 'sc=' + str(round(m['sc'], 3)))
    br.close()
json.dump(report, open(os.path.join(ROOT, 'render_report_blueprint.json'), 'w'), indent=2)
